...
`ksearch` 的目标是把每次搜索都变成未来可复用的本地知识资产。
...
基于 Chroma 或 Qdrant 的 KB 检索，不只是关键词匹配，而是语义召回。

### 支持迭代式 KB-first 搜索

先查 KB，再评估是否足够；只有本地知识不足时才扩展网页，并把新结果回灌到缓存和 KB。
...
KB 会记录 embedding model 和 dimension。切模型或维度时，如果和已有向量不一致，项目会阻止错误复用并要求显式 reset。
...
```bash
ksearch kb search "异步取消传播" --top-k 5
```

适合只想查本地 KB，不想触发新的网络抓取。
...
适合本地知识可能不完整，但又希望受控地扩展网页并回灌 KB 的场景。
...
- `--iterative` 必须和 `--kb chroma` 或 `--kb qdrant` 一起使用
...
- 修改 `embedding_model` 或 `embedding_dimension` 后，旧 KB 不能直接混用
- KB 元数据会持久化并在打开时校验
...
- `--no-cache`: 忽略缓存，强制联网
- `--only-cache`: 只返回缓存结果
- `--kb`: 启用 KB 检索，值为 `chroma`、`qdrant` 或 `none`
- `--embedding-model`: 指定 KB embedding 模型
- `--embedding-dimension`: 指定 KB embedding 维度
...
脚本会创建临时 KB 和测试文档，覆盖英文、中文和混合关键词流程，验证 `--only-cache` 与 `--iterative`，并生成 Markdown 报告。

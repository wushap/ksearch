...
**Goal:** Add a reusable end-to-end integration script that validates the live Ollama + ksearch + SearXNG workflow with multilingual keywords.
...
**Tech Stack:** Bash, pytest, ksearch CLI, Ollama HTTP API, SearXNG
...
- [ ] Add assertions that the script covers `kb reset`, `kb ingest`, `kb search`, `search --only-cache`, `search --iterative`, and a non-embedding model negative case.
...
- [ ] Build a standalone Bash script that validates live dependencies, creates an isolated temp KB, runs multilingual KB and search flows, and writes a markdown report.

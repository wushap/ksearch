...
**Goal:** 构建一个结合本地知识库缓存和网络搜索的 Python CLI 工具 `kbase`。
...
- [ ] **Step 2: 创建 src/kbase/__init__.py**

```python
"""kbase - Personal knowledge base with web search CLI tool."""
...
Run: `uv run python -c "import kbase; print(kbase.__version__)"`
Expected: `0.1.0`

- [ ] **Step 6: Commit**

```bash
git add pyproject.toml src/kbase/__init__.py tests/__init__.py
git commit -m "feat: initialize kbase project with uv"
```
...
```bash
git add src/kbase/models.py tests/test_models.py
...
```bash
git add src/kbase/config.py tests/test_config.py
...
```bash
git add src/kbase/cache.py tests/test_cache.py
...
```bash
git add src/kbase/searxng.py tests/test_searxng.py
...
```bash
git add src/kbase/converter.py tests/test_converter.py
...
```bash
git add src/kbase/output.py tests/test_output.py
...
from kbase.config import load_config, merge_config, DEFAULT_CONFIG, init_default_config
from kbase.cache import CacheManager
from kbase.searxng import SearXNGClient
from kbase.converter import ContentConverter
from kbase.search import SearchEngine
...
def search(
    ...
    # Execute search
    if verbose:
        console.print(Panel(f"Searching: {keyword}", title="kbase Search"))
...
Run: `uv run kbase search "test" --only-cache`
Expected: 搜索本地缓存（无结果时输出"无结果"）

- [ ] **Step 3: 验证配置文件初始化**

Run: `uv run python -c "from kbase.config import init_default_config; init_default_config('~/.kbase/config.json')"`
Expected: 配置文件已存在于 `~/.kbase/config.json`

- [ ] **Step 4: 验证网络搜索功能（可选，需要 SearXNG）**

Run: `uv run kbase search "python tutorial" -v`
...
```bash
git add -A
git commit -m "feat: complete kbase CLI tool implementation"
```
...
Plan complete and saved to `docs/superpowers/plans/2026-04-21-kbase-cli.md`. Two execution options:
